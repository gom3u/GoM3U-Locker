...
import { getDeviceInfo } from "./deviceinfo.js?v=1.0.8";
...
// Two simple, independent writes: bump the link's unlockCount, and log
// a device-info entry for the admin dashboard's device breakdown +
// per-link history. Neither blocks the user's unlock experience.
//
// SECURITY NOTE: this app is 100% client-side (no backend/Cloud
// Function), so a determined person could call these same Firestore
// writes directly from a browser console to inflate the count. A
// stricter session-token gate was tried but added enough round-trip
// writes that it became unreliable on slow/unstable mobile connections
// and made counts under-report for real users — worse than the fraud
// risk it guarded against. True elimination of that risk requires a
// Cloud Function. Documented as an accepted tradeoff.
async function recordUnlock() {
  const device = getDeviceInfo();

  try {
    await updateDoc(doc(db, "links", linkDocId), { unlockCount: increment(1) });
  } catch (err) {
    console.warn("Could not increment unlock count:", err);
  }
  ...
  try {
    ...
    console.warn("Could not log unlock device info:", err);
  }
}
