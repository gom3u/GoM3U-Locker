...
(function () {
  ...
  async function runDetection() {
    ...
    // Require at least 2 of 3 signals to agree before showing the
    // overlay. A single flaky/slow mobile network can cause one
    // external request (usually the Google AdSense fetch) to fail
    // even with no blocker present — a real blocker, by contrast,
    // blocks ad-pattern requests consistently and will trip multiple
    // signals at once.
    const blockedCount = [baitBlocked, adNetworkBlocked, decoyBlocked].filter(
      Boolean
    ).length;
